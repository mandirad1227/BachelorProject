...
def load():
    """
    Charge la DLL Dobot de manière portable (sans chemin absolu).

    - Windows  : charge 'DobotDll.dll' situé dans le même dossier que DobotDllType.py
                 (et ajoute ce dossier à la recherche de DLL).
                 Si introuvable, essaie l'env var DOBOT_SDK_DIR.
    - macOS    : charge 'libDobotDll.dylib' depuis le même dossier.
    - Linux    : charge 'libDobotDll.so' depuis le même dossier, sinon via le loader par défaut.
    """
    import platform, os
    ...
    if system == "Windows":
        # Messages d'origine conservés
        print("您用的dll是64位，为了顺利运行，请保证您的python环境也是64位")
        print("python环境是：", platform.architecture())

        # 1) Cherche DobotDll.dll à côté de ce fichier
        dll_path = here / "DobotDll.dll"

        # 2) Fallback : variable d'environnement DOBOT_SDK_DIR, si fournie
        if not dll_path.exists():
            env_dir = os.environ.get("DOBOT_SDK_DIR")
            if env_dir and (Path(env_dir) / "DobotDll.dll").exists():
                dll_path = Path(env_dir) / "DobotDll.dll"

        if not dll_path.exists():
            raise FileNotFoundError(
                f"DobotDll.dll introuvable. Emplacements testés :\n"
                f" - {here / 'DobotDll.dll'}\n"
                f" - $DOBOT_SDK_DIR/DobotDll.dll (si défini)\n"
                f"→ Place 'DobotDll.dll' à côté de DobotDllType.py ou définis DOBOT_SDK_DIR."
            )

        # Aide le loader Windows pour les dépendances éventuelles du DLL
        if hasattr(os, "add_dll_directory"):
            os.add_dll_directory(str(dll_path.parent))

        # Charge la DLL (RTLD_GLOBAL n'est pas nécessaire sous Windows)
        try:
            from ctypes import RTLD_GLOBAL  # peut ne pas exister sous Windows
            return CDLL(str(dll_path), RTLD_GLOBAL)
        except Exception:
            return CDLL(str(dll_path))

    elif system == "Darwin":  # macOS
        dylib = here / "libDobotDll.dylib"
        if not dylib.exists():
            raise FileNotFoundError(f"libDobotDll.dylib introuvable à : {dylib}")
        return CDLL(str(dylib))

    elif system == "Linux":
        so = here / "libDobotDll.so"
        if so.exists():
            return CDLL(str(so))
        # fallback: laisser le loader chercher dans le système
...
def GetEndEffectorSuctionCup(api):
    ...
    while True:
        ...
        if result != DobotCommunicate.DobotCommunicate_NoError:
            ...
            continue
        break
    return enableCtrl.value, isOn.value
